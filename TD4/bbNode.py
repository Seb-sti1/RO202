...
class BBNode:
    ...
    def branch(self, tree):
        ...
        if tree.bestSolution.shape[0] == 0 or (self.tableau.isMinimization and tree.bestObjective > self.tableau.bestObjective) \
                or (not self.tableau.isMinimization and tree.bestObjective < self.tableau.bestObjective):

            idx = -1
            for i in range(self.tableau.bestSolution.shape[0]):
                if isFractional(self.tableau.bestSolution[i]):
                    idx = i
                    break

            if idx == -1:  # aucune des variables de la solution n'est solution
                tree.bestObjective = self.tableau.bestObjective
                tree.bestSolution = self.tableau.bestSolution
            else:
                newA = np.array([0.0] * self.tableau.n)
                newA[idx] = 1
                newRhs = math.floor(self.tableau.bestSolution[idx])

                child_inf = BBNode.create_non_root(self, newA, newRhs)
                child_inf.branch(tree)

                newA = np.array([0.0] * self.tableau.n)
                newA[idx] = - 1
                newRhs = - math.ceil(self.tableau.bestSolution[idx])

                child_sup = BBNode.create_non_root(self, newA, newRhs)
                child_sup.branch(tree)
